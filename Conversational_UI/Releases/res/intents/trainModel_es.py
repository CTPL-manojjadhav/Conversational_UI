...
from module import stemmer_es
warnings.filterwarnings("ignore")
vectorDimension=200
domain=sys.argv[1]
scriptDir=os.path.dirname(__file__)
fileData=os.path.join(scriptDir,'data',domain+'_es.json')
utterance=[]
intent=[]
...
stopListFile=os.path.join(scriptDir,'..','dictionary','stopwords_es.txt')
arrayWords=[]
stopWords=[]
...
tfidfVec=TfidfVectorizer(utterance,decode_error='ignore',stop_words=stops,ngram_range=(1,5),tokenizer=stemmer_es.stemTokenize_1)
...
pickle_path=os.path.join(scriptDir,'model',domain+'_es_')
